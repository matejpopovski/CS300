...
public class ArtGallery {
  ...
  public boolean lookup(String name, int year, double cost) {
    ...
    // Hint: create a new artwork with the provided name and year and default cost and use it in the
    // search operation
    Artwork search = new Artwork(name, year, cost);
    boolean check = lookupHelper(search, root);



    // TODO complete the implementation of this method
    return check; // Default return statement added to resolve compiler errors
    // TODO complete the implementation of this method
  ...
  protected static boolean lookupHelper(Artwork target, BSTNode<Artwork> current) {
    if (current != null) {
      if (target.compareTo(current.getData()) < 0) {
        ...
        return lookupHelper(target, current);
  ...
  public boolean addArtwork(Artwork newArtwork) {
    // TODO complete the implementation of this method
    if (newArtwork == null) {
      throw new NullPointerException("the given node is empty");
    }
    if (size == 0) {
    ...
    boolean check = addArtworkHelper(newArtwork, root);
    if (check == true) {
      size++;
    }
    return check;
  ...
  protected static boolean addArtworkHelper(Artwork newArtwork, BSTNode<Artwork> current) {
    BSTNode<Artwork> nee = new BSTNode<Artwork>(newArtwork);
    if (lookupHelper(newArtwork, current) == true) {
      return false;
    }
   ...
   */
  public int height() {
    int toReturn = heightHelper(root);
  ...
  protected static int heightHelper(BSTNode<Artwork> current) {
    if (current == null) {
      ...
      int depthOne = heightHelper(current.getLeft());
      int depthTwo = heightHelper(current.getRight());
  ...
  public ArrayList<Artwork> lookupAll(int year, double cost) {
    ...
    ArrayList<Artwork> toReturn = lookupAllHelper(year, cost, current);
  ...
  protected static ArrayList<Artwork> lookupAllHelper(int year, double cost,
    ...
    if (current.getLeft() != null) {
      toReturn.addAll(lookupAllHelper(year, cost, current.getLeft()));
    }
    if (current.getRight() != null) {
      toReturn.addAll(lookupAllHelper(year, cost, current.getRight()));
    }
  ...
  protected static BSTNode<Artwork> buyArtworkHelper(Artwork target, BSTNode<Artwork> current)
    ...
    if (target.compareTo(current.getData()) < 0) {
      ...
      if (current.getLeft() == null && current.getRight() == null) {
        ...
        BSTNode<Artwork> newNode = new BSTNode<Artwork>(getSuccessor(current));
        BSTNode<Artwork> newRightNode = new BSTNode<Artwork>(current.getRight().getData());
        if (newNode.getData().compareTo(newRightNode.getData()) == 0) {
          current = current.getRight();
          return current;
        }
        buyArtworkHelper(getSuccessor(current), current);
        current = newNode;
  ...
  protected static Artwork getSuccessor(BSTNode<Artwork> node) {
    BSTNode<Artwork> parent = node.getRight();
